...
Real VLA는 갑자기 등장한 단일 모델 계열이 아니다. 현재의 VLA는 classical control, state estimation, visual perception, 3D reconstruction, symbolic planning, motion planning, robot learning, VLM/LLM, robot data scaling, action representation 연구가 겹치면서 만들어진 문제 설정이다. 표~\ref{tab:ch0-history-timeline}는 이 책이 어떤 역사적 흐름을 하나의 engineering contract로 다시 읽는지 요약한다.
...
이 정책은 최신성을 낮추기 위한 장치가 아니다. 오히려 최신 VLA 흐름을 더 정확히 읽기 위한 장치다. 예를 들어 action representation 개선 논문은 Chapter 13의 control boundary 관점에서 매우 중요하지만, 그 claim이 real robot task, simulation task, throughput benchmark 중 어디에서 닫혔는지는 별도로 기록해야 한다.
...
\begin{enumerate}
  \item \textbf{Robotics foundation}: state, dynamics, estimation, control, contact, planning을 Real VLA의 실행 조건으로 재정리한다.
...
\node[stage, below=of ch0] (foundation) {\textbf{Ch2--Ch5: Robotics foundation}\\
state, estimation, control, contact, planning이 action을 물리 실행으로 바꾸는 조건을 만든다.};
\node[stage, below=of foundation] (learning) {\textbf{Ch6--Ch8: Learning foundation}\\
imitation learning, RL, visuomotor policy가 language-conditioned action policy의 직접 기반이 된다.};
\node[stage, below=of learning] (language) {\textbf{Ch9--Ch12: Language/model bridge}\\
transformer, VLM, LLM planner, planner+skills가 VLA로 넘어가는 구조적 경로를 제공한다.};
\node[stage, below=of language] (core) {\textbf{Ch13--Ch16: VLA system core}\\
action representation, data engine, adaptation, evaluation이 model output을 system claim으로 바꾼다.};
\node[stage, below=of core] (deploy) {\textbf{Ch17--Ch22: Deployment and assurance}\\
...
3 & Control Interfaces & policy action과 controller command의 차이를 고정한다. & $\act_t \rightarrow \ctrlcmd_t$ interface \\
...
8 & Visuomotor Policies & language가 붙기 전 image-to-action policy의 구조와 한계를 정리한다. & observation-to-action backbone 관점 \\
...
12 & Robotics Transformer Era & RT 계열과 multi-embodiment learning이 action token 관점을 만든 흐름을 정리한다. & sequence prediction으로서의 action \\
...
17 & Hierarchical VLA and Embodied Reasoning & 단일 policy가 아니라 reasoning layer와 action layer가 나뉘는 구조를 다룬다. & hierarchy, monitor, replan trigger \\
...
\chapterwork
{Chapter 1의 Real VLA definition, Chapter 13의 action contract, Chapter 16의 evaluation vector를 함께 읽어라. 세 장이 이 책의 기본 판정 기준이다.}
