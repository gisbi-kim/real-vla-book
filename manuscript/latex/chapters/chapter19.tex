...
이 hierarchy는 Chapter 18의 계층형 VLA와 직접 연결된다. 상위 reasoning은 느려도 된다. 그러나 safety monitor와 controller는 느리면 안 된다. VLA가 직접 50 Hz action을 내는 구조와, VLA가 5 Hz action chunk를 내고 low-level controller가 500 Hz로 추적하는 구조는 완전히 다른 design point이다.
