...
더 직접적으로 말하면, 이 책은 로봇공학자의 눈으로 VLA claim을 검증하기 위한 책이다. VLA 연구는 중요하고 앞으로 더 중요해질 것이다. 다만 image-language reasoning, benchmark score, demo success만으로 실제 robot capability가 증명되지는 않는다. Semantic success는 곧바로 robot capability가 아니다. Real VLA claim은 model output이 실제 로봇 실행, 평가, 안전으로 닫히는지를 함께 보여주어야 한다.

이 책이 경계하는 것은 VLA 자체가 아니다. 이 책이 경계하는 것은 VLA를 읽는 과정에서 robotics interface가 사라진 듯 보이는 독해이다. VLM/VLA가 물체 이름, 장면 설명, high-level instruction을 잘 처리해도 로봇은 여전히 pose, uncertainty, contact, controller rate, actuator limit, reset, failure label, unsafe event 안에서 움직인다. 따라서 이 책은 ``어떤 모델인가?''보다 ``그 모델의 action이 어떤 controller contract와 safety/evaluation evidence로 실제 behavior가 되는가?''를 먼저 묻는다. 독자는 이 책을 통해 VLA 논문을 computer vision/ML 성능표가 아니라 robot system claim으로 읽는 기준을 얻어야 한다.
