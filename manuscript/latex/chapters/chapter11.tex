...
\end{equation}
여기서 $\mathcal{C}$는 controller, motion planner, safety checker의 집합이다. 이 구조는 Chapter 6의 planning과 Chapter 4의 control interface를 language model과 연결한 형태이다.
...
\end{equation}
이 형식이 있으면 LLM은 단순히 ``실패했다''가 아니라 ``grasp approach stage에서 normal force가 threshold보다 낮았다'' 같은 정보를 사용할 수 있다. 이것이 Chapter 20의 safety and assurance와 연결된다.
