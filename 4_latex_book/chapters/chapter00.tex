...
Real VLA는 갑자기 등장한 단일 모델 계열이 아니다. 현재의 VLA는 classical control, state estimation, symbolic planning, motion planning, robot learning, VLM/LLM, robot data scaling, action representation 연구가 겹치면서 만들어진 문제 설정이다. 표~\ref{tab:ch0-history-timeline}는 이 책이 어떤 역사적 흐름을 하나의 engineering contract로 다시 읽는지 요약한다.
